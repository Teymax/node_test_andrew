...
const updateOrCreate = async (usercheck) => {
    ...
    [err, user] = await to(User.findAll({where: {email: usercheck.email}}));
    user = user[0];
    ...
    [err, user] = await to(user.update({username: usercheck.name}));
    [err, user] = await to(user.update({email: usercheck.email}));
    return user;
}
...
const loginFacebook = async (req, res) => {
    ...
    let user = {
        ...
        family_name: req.user._json.name,
        email: req.user._json.email,
        provider: req.user.provider
    };
